...
- Because the company project requirements, must put x264 video compiled files are added to the ffmepg repository
- 因公司项目需求，必须把x264视频编译文件添加到ffmepg库里面
- Oneself fumble with git lookup can made a compilation of x264 library
- 自己摸索加git查找做了一个能编译x264库
- To compile x264 library into ffmpeg, and need to compile x264 library
- 要把x264库编译进入ffmpeg里面，需要先编译x264库
- Will need x264 before compilation of ffmpeg, relevant agreements in ffmpeg configuration file to open
- 在编译ffmpeg之前需要将x264，相应协议在ffmpeg配置文件里面打开（配置已被打开 --enable-decoder=libx264）
